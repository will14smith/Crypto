...
namespace Crypto.IO.TLS
{
    ...
    {
        public PlaintextRecord(RecordType type, TlsVersion version, ushort length)
            : this(type, version, length, new byte[length])
        {
        }

        public PlaintextRecord(RecordType type, TlsVersion version, ushort length, byte[] fragment)
